"""
ingest_lead_actresses.py
========================
Sprint 24 — Ingest South Indian lead actresses.

Adds the gender='F' + is_primary_actor=TRUE combination that, together with
the existing is_primary_actor=TRUE + gender='M' male leads, completes the
three-category actor classification:

  1. Lead Actors   : is_primary_actor=TRUE  AND gender='M'
  2. Lead Actresses: is_primary_actor=TRUE  AND gender='F'
  3. Supporting    : is_primary_actor=FALSE  (gender usually NULL)

Two groups of actresses are handled:

  Group A — Already in DB as supporting cast (discovered via network/TMDB
             ingestion in earlier sprints).  Promoted to is_primary_actor=TRUE,
             gender set to 'F', and industry corrected where misclassified.

  Group B — Not yet in DB.  Inserted fresh with is_primary_actor=TRUE and
             gender='F'.

For every actress (both groups) the script fetches her complete TMDB
filmography and upserts actor_movies rows, filling all the films she has
appeared in that already exist in the movies table.
...
-----
  ...
  python -m data_pipeline.ingest_lead_actresses
  python -m data_pipeline.ingest_lead_actresses --dry-run
  python -m data_pipeline.ingest_lead_actresses --actor "Nayanthara"
  python -m data_pipeline.ingest_lead_actresses --skip-analytics
...
ACTRESSES_TO_PROCESS: list[tuple[str, str, int]] = [
    # ── Group A: In DB as supporting — promote + set gender='F' ───────────
    # Tamil
    ("Nayanthara",             "Tamil",     91548),
    ("Trisha Krishnan",        "Tamil",     116925),
    ("Jyotika",                "Tamil",     147324),
    ("Sai Pallavi",            "Tamil",     1473119),
    ("Keerthy Suresh",         "Tamil",     1295762),
    ("Meena",                  "Tamil",     549602),
    ("Revathi",                "Tamil",     584324),
    ("Shobana",                "Tamil",     110298),
    # Telugu
    ("Samantha Ruth Prabhu",   "Telugu",    225312),
    ("Tamannaah Bhatia",       "Telugu",    85721),
    ("Anushka Shetty",         "Telugu",    88167),
    ("Pooja Hegde",            "Telugu",    587753),
    ("Rashmika Mandanna",      "Telugu",    1752056),
    # Malayalam (Parvathy & Nazriya were misclassified as Tamil in DB)
    ("Parvathy Thiruvothu",    "Malayalam", 1053894),   # was: industry=Tamil
    ("Nazriya Nazim",          "Malayalam", 5669766),   # was: industry=Tamil
    ("Manju Warrier",          "Malayalam", 1178639),
    ("Urvashi",                "Malayalam", 141702),
]
...
def ingest_lead_actresses(
    ...
    print(f"\n{_SEP_BOLD}")
    print(f"  Sprint 24 — Lead Actress Ingestion{mode}")
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Ingest South Indian lead actresses (Sprint 24).",
...
if __name__ == "__main__":
    args = _parse_args()
    sys.exit(ingest_lead_actresses(
